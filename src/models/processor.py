"""
This file is copied from https://github.com/som-shahlab/femr.
We had to add a .cpu() call in line 327 to make this code work on a GPU device
"""
...
def map_preliminary_batch_stats(
    batch, indices, *, processor: FEMRBatchProcessor, max_length: int
):
    """
    ...
    for patient_index, patient_id, events in zip(
        indices, batch["patient_id"], batch["events"]
    ):
        patient = {
        ...
        if data["needs_exact"]:
            ...
            lengths.append(
                (patient_index, current_start, current_end - current_start + 1)
            )
        else:
...
class BatchCreator:
    ...
    def add_patient(
        self, patient: meds.Patient, offset: int = 0, max_length: Optional[int] = None
    ):
        ...
        for event in patient["events"]:
            ...
            for measurement in event["measurements"]:
                ...
                features, weights = self.tokenizer.get_feature_codes(
                ...
                if not self.tokenizer.is_hierarchical:
                    ...
                    per_patient_token_indices.append(
                        len(per_patient_hierarchical_tokens)
                    )

                per_patient_ages.append(
                    (event["time"] - birth) / datetime.timedelta(days=1)
                )
                per_patient_normalized_ages.append(
                    self.tokenizer.normalize_age(event["time"] - birth)
                )
                per_patient_timestamps.append(
                    event["time"].replace(tzinfo=datetime.timezone.utc).timestamp()
                )
        ...
        self.normalized_ages.extend(
            per_patient_normalized_ages[offset : offset + length_to_add]
        )
        ...
        if not self.tokenizer.is_hierarchical:
            ...
            self.token_indices.extend(
                [
                    ...
                    for value in per_patient_token_indices[
                        offset + 1 : offset + length_to_add + 1
                    ]
                ]
            )

            self.hierarchical_tokens.extend(
                per_patient_hierarchical_tokens[internal_start:internal_end]
            )
            self.hierarchical_weights.extend(
                per_patient_hierarchical_weights[internal_start:internal_end]
            )
    ...
    def get_batch_data(self):
        ...
        if not self.tokenizer.is_hierarchical:
            ...
            transformer["hierarchical_tokens"] = np.array(
                self.hierarchical_tokens, dtype=token_dtype
            )
            transformer["hierarchical_weights"] = np.array(
                self.hierarchical_weights, dtype=np.float16
            )
    ...
    def cleanup_batch(self, batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        batch["transformer"]["patient_lengths"] = np.array(
            batch["transformer"]["patient_lengths"].cpu()
        )
...
class FEMRBatchProcessor:
    ...
    def convert_patient(
        ...
        **formatter_kwargs,
        ...
        if tensor_type is not None:
            formatter = datasets.formatting.get_formatter(
                tensor_type, **formatter_kwargs
            )
    ...
    def collate(self, batches: List[Mapping[str, Any]]) -> Mapping[str, Any]:
        """
        A collate function that prepares batches for being fed into a DataLoader.
        This function is updated to handle multiple batches, which is necessary
        when using multiple GPUs.
        """
        if len(batches) > 1:
            # Merge multiple batches into one
            merged_batch = self.merge_batches(batches)
            return {"batch": _add_dimension(self.creator.cleanup_batch(merged_batch))}
        else:
            return {"batch": _add_dimension(self.creator.cleanup_batch(batches[0]))}

    def merge_batches(self, batches: List[Mapping[str, Any]]) -> Mapping[str, Any]:
        """
        Merges a list of batches into a single batch. This is required when using
        multiple GPUs to ensure each device gets the correct portion of the batch.
        """
        merged_batch = {}

        for key in batches[0]:
            if isinstance(batches[0][key], torch.Tensor):
                # Collect all tensors to concatenate
                tensors = [batch[key] for batch in batches]

                # Check if tensors are zero-dimensional and convert them to one-dimensional
                tensors = [
                    tensor.unsqueeze(0) if tensor.dim() == 0 else tensor
                ...
                # Collect all arrays to concatenate
                arrays = [batch[key] for batch in batches]

                # Check if arrays are zero-dimensional and convert them to one-dimensional
                arrays = [
                    np.expand_dims(array, axis=0) if array.ndim == 0 else array
                    for array in arrays
                ]

                # Concatenate along the first dimension
                merged_batch[key] = np.concatenate(arrays, axis=0)
            elif isinstance(batches[0][key], list):
                # For lists, simply extend them
                merged_batch[key] = []
                for batch in batches:
                    merged_batch[key].extend(batch[key])
            else:
                # For other types (e.g., scalars), just take the first one (assuming they are the same across batches)
                merged_batch[key] = batches[0][key]
    ...
    def convert_dataset(
        ...
        if isinstance(dataset, datasets.DatasetDict):
            return datasets.DatasetDict(
                {
                    k: self.convert_dataset(
                        v, tokens_per_batch, min_patients_per_batch, num_proc
                    )
        ...
        lengths = femr.hf_utils.aggregate_over_dataset(
            dataset,
            functools.partial(
                map_preliminary_batch_stats, processor=self, max_length=max_length
            ),
