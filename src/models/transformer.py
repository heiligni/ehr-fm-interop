...
def fixed_pos_embedding(ages, dim, dtype):
    ...
    inv_freq = 1.0 / (
        10000 ** (torch.linspace(0, 2, steps=dim // 2, device=ages.device))
    )
...
class FEMRTransformer(nn.Module):
    def __init__(self, config: femr.models.config.FEMRTransformerConfig):
        ...
        self.layers = nn.ModuleList(
            [FEMREncoderLayer(config) for _ in range(self.config.n_layers)]
        )
    ...
    def forward(self, batch):
        if not self.config.is_hierarchical:
            ...
            x = self.embed_bag(
                batch["hierarchical_tokens"],
                batch["token_indices"],
                batch["hierarchical_weights"],
            )
...
class MOTORTaskHead(nn.Module):
    ...
    def forward(
        ...
        event_loss = (
            -math.log(2)
            * torch.where(batch["is_event"], time_dependent_logits, 0).mean()
        )
...
def compute_features(
    ...
    model = FEMRModel.from_pretrained(model_path, task_config=task.get_task_config())
    # tokenizer = femr.models.tokenizer.FEMRTokenizer.from_pretrained(model_path, ontology=ontology)
    processor = FEMRBatchProcessor(tokenizer, task=task)
